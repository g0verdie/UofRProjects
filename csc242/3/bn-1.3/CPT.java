 ...
 * Time-stamp: <Tue Mar 27 20:59:46 EDT 2012 ferguson>
 */
 ...
 * <p>
 * This implementation uses tree of Entries. The root Entry is for the first
...
public class CPT {
     ...
     */
    abstract protected class Entry {
     ...
     */
    protected class Dimension extends Entry {
     ...
     * Entry.
     */
    protected class ProbabilityValue extends Entry {
	public double value;
    ...
    /**
     * Construct and return a new CPT for the given query and <q>given</q>
     * variables, initialized with probabilities from the given string.
     * This constructor is used when creating CPTs from XMLBIF files.
     * @see CPT#initFromString
     */
    public CPT(RandomVariable query, List<RandomVariable> givens, String str) {
	this(query, givens);
	initFromString(str);
    }
...
A1 A2 A3  A1 A2 A3  A1 A2 A3  A1 A2 A3    A1 A2 A3  A1 A2 A3  A1 A2 A3  A1 A2 A3
     ...
     */
    protected Iterator<ProbabilityValue> valueIterator() {
     ...
     */
    public void initFromString(String str) throws NumberFormatException, CPTFormatException {
	//trace("initfromString: " + str);
	StringTokenizer tokens = new StringTokenizer(str);
	Iterator<ProbabilityValue> values = valueIterator();
	while (tokens.hasMoreTokens()) {
	    String token = tokens.nextToken();
	    //trace("probability: " + token);
	    ProbabilityValue pv = values.next();
