...
public class LikelihoodWeighting extends InferenceRG{
	double w;
	
	public LikelihoodWeighting (BNode query,BayesianNetworkRG bnet) 
	{super(query,bnet);}
	
	public LikelihoodWeighting(BNode query,String evidence,BayesianNetworkRG bnet) 
	...
	public double[] rejAsk() {
		...
		for(int i = 0; i < 1000000; i++) {
				this.priorSample();
				probEach[query.getDomain().indexOf(query.getVal())]+=w;
	...
	public void priorSample() {
		...
		for(int i = 0; i < nodes.size(); i++) {	
			if(this.evidenceVars.contains(nodes.get(i))) {
				w *= nodes.get(i).getCPT().findSpot(nodes.get(i).getParents());
	...
	public static void main(String[] args) throws IOException, ParserConfigurationException, SAXException {
		...
		if(args.length > 2) {
			...
			lw = new LikelihoodWeighting (query,args[2],network);
		}
		else{
			lw = new LikelihoodWeighting (query,network);
		}
		//get answer
		lw.answer = lw.rejAsk();
		lw.printQuery();
